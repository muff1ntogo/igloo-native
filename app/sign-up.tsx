...
export default function SignUpScreen() {
  ...
  const handleSignUp = useCallback(async () => {
    ...
    try {
      ...
      router.push({
        pathname: "/verify-otp",
        params: { flow: "signup", email: encodeURIComponent(e) },
      });
    } catch (err: unknown) {
    ...
    <SafeAreaView className="flex-1 bg-background px-8 justify-center">
      ...
      <View className="w-full space-y-4">
        ...
        <TouchableOpacity
          ...
          disabled={loading}
          className="h-14 rounded-2xl bg-primary items-center justify-center mt-2 shadow-md active:opacity-90 mb-4"
        >
        ...
        >
          <Text className="font-sans text-sm text-primary">Already have an account? Log in</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
    </KeyboardDismissView>
